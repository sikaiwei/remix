...
contract CSAMM {
    ...
    function swap(address _tokenIn, uint _amountIn) external returns(uint amountOut) {
        ...
        (IERC20 tokenIn, IERC20 tokenOut, uint resIn, uint resOut) =
        ...
        (uint res0, uint res1) = isToken0 ? (resIn + _amountIn, resOut - amountOut) : (resIn -amountOut, resOut + _amountIn);
        _update(res0, res1);
        // transfer token out
    ...
    function addLiquidity(uint _amount0, uint _amount1) external returns(uint shares) {
        ...
        s should be proprotional to increase from L to L + a
        (L + a) / L = (T + s) / T
